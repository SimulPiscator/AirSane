...
_(kDNSServiceErr_Unknown)//                    = -65537,    // 0xFFFE FFFF (first error code)
...
//    _(kDNSServiceErr_MemFree)//                    = -65792    // 0xFFFE FF00 (last error code)
#undef _
    { 0, nullptr }
};
...
struct ServiceEntry
{
    ...
    ServiceEntry(MdnsPublisher::Service* p)
        : mpService(p), mDNSServiceRef(nullptr)
    {
    }
    ...
    bool announce()
    {
        ...
        for (const auto& entry : mpService->txtRecord()) {
            DNSServiceErrorType err = ::TXTRecordSetValue(&txtRecord, entry.first.c_str(), entry.second.length(), entry.second.data());
            if (err != kDNSServiceErr_NoError) {
                ok = false;
                std::cerr << "Could not add txtRecord value " << entry.first << "=" << entry.second << " (" << dnssd_strerr(err) << ")" << std::endl;
            }
        }
        ...
        if (err != kDNSServiceErr_NoError) {
            ok = false;
            std::cerr << "Could not register service " << mpService->name() << " (" << dnssd_strerr(err) << ")" << std::endl;
        }
        else {
