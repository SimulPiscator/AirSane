...
Dictionary &PdfEncoder::documentInfo()
{
    return p->mInfoDict;
}

const Dictionary &PdfEncoder::documentInfo() const
{
    return p->mInfoDict;
}
...
/Type/Page
/Contents 6 0 R
/MediaBox [ 0 0 )" << pdfunits_per_px*width() << " " << pdfunits_per_px*height() << R"( ]
/Parent 1 0 R
...
/ColorSpace /)" << csname << R"(
/BitsPerComponent )" << bitDepth() << R"(
/Length )" << height()*bytesPerLine() << R"(
>>
stream
)";
}
